...
- `src/main/resources` contains `META-INF/mods.toml` and assets. Use `assets/heavy_storms/{blockstates,models,lang}` for data-driven content; register textures under `textures/block` and `textures/item`.
...
- `.\gradlew.bat --no-daemon build` compiles, runs ForgeGradle processors, and produces jars in `build/libs`.
- `.\gradlew.bat --no-daemon runClient` launches the Forge dev client for in-game verification; the first run downloads dependencies.
...
- Target Java 17 with 4-space indentation and same-line braces; mark classes `final` when not intended for extension.
...
- Log debug info through the Forge logger, but strip noisy entries before committing.
